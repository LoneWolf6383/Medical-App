...
export const HeartDisease = (props) => {
    ...
    const handleSubmit = async () => {
        ...
        const { data: res } = await axios.post('http://localhost:3001/verifyHeartDisease', input)
        console.log(res);
        res===1 && setFlag(1)
        res===2 && setFlag(2)
    }
    return (
        ...
        <NavBar/>   
            ...
            <Form>
                ...
                {
                        symptoms.map(s => (!(s.toLowerCase() in {'age':'','_id':'','gender':'','outcome':'','sex':''} )&&
                ...
                { flag===1 && <p>You have No symptoms of Heart Disease</p>}
                { flag===2 && <p>You have symptoms of Heart Disease</p>}
            </div>
        </div>
  )
}
